...
# 課題の為の修正内容
# 回答精度向上のため、
# 会話生成と評価処理を分離し、評価専用Chainを新たに設計した。

# 評価時には会話履歴を使用せず、問題文・回答文・学習者レベルのみを入力として与えることで、
# 文脈ノイズを排除し、評価の一貫性と精度を向上させている。
# また、学習者レベルをプロンプトに明示的に組み込むことで、
# フィードバック内容が学習段階に応じて変化する設計とした。